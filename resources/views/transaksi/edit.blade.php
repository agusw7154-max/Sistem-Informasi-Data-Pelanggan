...
<form method="POST" action="{{ route('transaksi.update', $transaksi->ID_TRANSAKSI) }}">
    ...
    <div class="mb-3">
        ...
        <input class="form-control" name="ID_ADMIN" value="{{ old('ID_ADMIN', $transaksi->ID_ADMIN) }}" required>
    </div>
    <div class="mb-3">
        <label class="form-label">ID Pelanggan</label>
        <input class="form-control" name="ID_PELANGGAN" value="{{ old('ID_PELANGGAN', $transaksi->ID_PELANGGAN) }}" required>
    </div>
    <div class="mb-3">
        <label class="form-label">ID Pembayaran</label>
        <input class="form-control" name="ID_PEMBAYARAN" value="{{ old('ID_PEMBAYARAN', $transaksi->ID_PEMBAYARAN) }}">
    </div>
    <div class="mb-3">
        <label class="form-label">Tanggal Transaksi</label>
        <input class="form-control" type="datetime-local" name="TANGGAL_TRANSAKSI" value="{{ old('TANGGAL_TRANSAKSI', $transaksi->TANGGAL_TRANSAKSI) }}">
    </div>
...
<script>
    ...
    const existing = @json($transaksi->detailTransaksis->map(function($d){ return ['ID_PRODUK'=>$d->ID_PRODUK, 'JUMLAH'=>$d->JUMLAH]; }));
    let idx = 0;
    ...
    function addRow(item = null){
        ...
        // If this row is an existing item, disable product select and add hidden input so the product id is submitted
        if(item){
            const select = tr.querySelector('.prod-select');
            const hidden = document.createElement('input');
            hidden.type = 'hidden';
            hidden.name = select.name; // same name so server receives it
